% Y存储公式中需要的符号(21*n)，下面用的是线性索引
function dY = uavWithObserverAndControllerDynamics(t, Y, params, neighbors_in)
    % 说明：初始化（目标位置、群中心、无人机位置与速度）由生成脚本完成；
    % 本函数只计算状态导数，通信拓扑按 r_c 在线生成。

    % --- 读入/尺寸 ---
    N = size(neighbors_in, 1); % 仅用来获得N
    K = 21;
    dY = zeros(N * K, 1);
    ...
    % 目标运动类型：'line' 或 'curve'；直线用 params.target_v = [vx,vy,vz]
    ...
    neighbors = convert_adjacency_matrix(neighbors); % 你的原工具
